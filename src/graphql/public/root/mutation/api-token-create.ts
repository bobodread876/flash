...
const ApiTokenCreateInput = GT.Input({
  ...
  fields: () => ({
    name: {
      ...
      description:
        "A descriptive name for the API token (e.g., 'BTCPayServer Integration')",
    },
    scopes: {
      type: GT.List(GT.NonNull(ApiTokenScope)),
      defaultValue: ["read:user"],
      description: "Permission scopes for the token. Defaults to read-only user access.",
    },
    expiresIn: {
      type: GT.Int,
      description:
        "Optional expiration time in seconds. If not set, token doesn't expire.",
    },
  }),
})
      ...
      type: GT.Object({
        ...
        fields: () => ({
          ...
          token: {
            type: GT.NonNull(GT.String),
            description:
              "The actual API token. Store this securely as it won't be shown again.",
          },
...
const ApiTokenCreateMutation = GT.Field({
  ...
  description:
    "Generate a new API token for external service authentication. The token is only shown once and cannot be retrieved later.",
  args: {
    input: {
